  ...
  static initialize(sequelize: Sequelize) {
    User.init({
      ...
      email: {
        ...
        allowNull: false,
        validate: {
          isTenantEmail(value: string) {
            if (!/^[^@\s]+@[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/i.test(value)) {
              throw new Error("E-mail deve estar no formato usuario@slugdapousada.");
            }
          },
        },
      },
      passwordHash: { type: DataTypes.STRING(255), allowNull: false, field: "password_hash" },
      role: { type: DataTypes.ENUM("admin", "staff", "customer"), allowNull: false, defaultValue: "admin" },
      tenantId: { type: DataTypes.INTEGER.UNSIGNED, allowNull: false, field: "tenant_id", references: { model: "tenants", key: "id" } },
      phone: { type: DataTypes.STRING(30), allowNull: true },
      teamRole: { type: DataTypes.ENUM("Recepcao", "Limpeza", "Manutencao", "Gestao"), allowNull: false, defaultValue: "Recepcao", field: "team_role" },
      employmentStatus: { type: DataTypes.ENUM("active", "inactive"), allowNull: false, defaultValue: "active", field: "employment_status" },
      shiftStatus: { type: DataTypes.ENUM("off", "on_shift"), allowNull: false, defaultValue: "off", field: "shift_status" },
