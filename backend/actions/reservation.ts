  ...
  try {
    return await sequelize.transaction(async (transaction) => {
      ...
      if (minimumStay.nights > 0 && stayLength < minimumStay.nights) {
        throw new Error(
          `Este quarto exige pelo menos ${minimumStay.nights} noites.`,
        );
      }

      if (minimumStay.days > 0 && stayLength < minimumStay.days) {
        throw new Error(
          `Este quarto exige pelo menos ${minimumStay.days} dias.`,
        );
      }

      const blockingClosure = findBlockingClosure(
        roomPolicy,
        checkIn,
        checkOut,
      );
      if (blockingClosure) {
        throw new Error(
          "Este quarto está fechado ou bloqueado para o período informado.",
        );
      }
      ...
      if (overlappingReservations.length >= room.quantity) {
        throw new Error(
          "Não há unidades disponíveis deste quarto para este período.",
        );
      }
      ...
      if (input.preferredUnitNumber !== undefined) {
        if (
          ...
          throw new Error(
            `Unidade inválida. Este quarto tem ${room.quantity} unidade(s).`,
          );
        }

        if (takenUnits.has(input.preferredUnitNumber)) {
          throw new Error(
            `A unidade ${input.preferredUnitNumber} já está ocupada nesse período.`,
          );
        }
      ...
      if (input.recomputePrice) {
        const pricePerNight = getRoomEffectivePrice(
          {
            ...
            seasonalRates: roomPolicy.seasonalRates as RoomSeasonalRate[],
          },
          checkIn,
        );
        ...
        if (input.couponCode) {
          ...
          computedAmount =
            computedAmount * (1 - Number(coupon.discountPercentage) / 100);
      ...
      const created = await Reservation.create(
        {
          ...
          status:
            input.entryType === "manual_reservation" ? "confirmed" : "blocked",
          channelReference:
            input.entryType === "manual_reservation"
              ? "MANUAL-RES"
              : "MANUAL-BLOCK",
    ...
    if (error instanceof UniqueConstraintError && input.paymentReference) {
      ...
      if (existing) {
        ...
        return mapReservationToDomain(
          existing,
          room?.localRoomId ?? input.roomId,
        );
      }
    }
...
export async function createPublicReservation(
  input: Omit<
    ReservationCreationContext,
    "tenantId" | "amount" | "recomputePrice"
  > & { tenantId: number },
) {
  if (
    !input.roomId ||
    ...
    throw new Error(
      "Todos os campos são obrigatórios para concluir a reserva.",
    );
  }
